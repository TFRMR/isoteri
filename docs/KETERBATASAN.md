...
- **Target WebAssembly sudah berhasil dibangun & divalidasi** (build dilakukan di mesin lokal dengan Rust terpasang lewat `rustup`, bukan di environment pengembangan sandbox — sandbox itu sendiri tidak punya `rustup` dan tidak ada akses jaringan ke `static.rust-lang.org`, jadi tidak bisa dipakai untuk langkah build ini). Hasilnya: `isoteri-wasm/` di-compile ke `wasm32-unknown-unknown` lewat `wasm-pack`, lalu diverifikasi jalan lewat `runtime/web/demo_wasm.html` (compile source `.iso` langsung di browser, tanpa CLI). Yang masih terbatas: baru diuji fitur bahasa sederhana (fungsi, string, aritmatika) — belum divalidasi lewat jalur WASM untuk struct, closure, loop, atau DOM binding. Siapa pun yang mau membangun ulang `pkg/` butuh Rust terpasang lewat `rustup` (bukan `apt`) di mesinnya sendiri — lihat `isoteri-wasm/README.md` untuk langkahnya.
